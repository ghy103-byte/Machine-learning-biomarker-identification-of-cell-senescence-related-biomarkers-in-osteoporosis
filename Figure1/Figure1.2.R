...
pdf(file="Before scaled.PDF", width=10, height=6)
...
pdf(file="After scaled.PDF", width=10, height=6)
boxplot(express.norm,outline = F,col =cols) 
dev.off()
write.csv(express.norm,file = "merge matrix new.csv",row.names = T)
...
write.csv(x, "result for DEGs.csv", quote = F)
...
logFC_filter=0.15
P.Value_filter=0.05
merge<-read.csv("result for DEGs.csv",header = T,sep = ",")
colnames(merge)[1]<-"ID"
...
SENESCENCE=read.table("SENESCENCE.txt",header=T)
SENESCENCE=SENESCENCE[,1]

library(VennDiagram)
library(RColorBrewer)

aaa<-list(merge_UP=merge_UP , SENESCENCE=SENESCENCE )
...
aaa<-list(merge_DOWN=merge_DOWN , SENESCENCE=SENESCENCE)
...
a<-intersect(merge_UP,SENESCENCE)


upgene<-unique(c(a))

d<-intersect(merge_DOWN,SENESCENCE)
...
write.table(difgene,"ALL DEGs.txt",row.names = F,quote = F)
write.table(upgene,"up genes.txt",row.names = F,quote = F)
write.table(downgene,"down genes.txt",row.names = F,quote = F)
difgeneall<-c(merge_UP,merge_DOWN)
write.table(difgeneall,"DEGs.txt",row.names = F,quote = F)
