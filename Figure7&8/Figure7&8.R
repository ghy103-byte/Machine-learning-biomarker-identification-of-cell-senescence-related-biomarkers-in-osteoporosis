...
library(ggradar)
rm(list = ls())
FF<-read.csv("merge matrix new.csv",sep = ",",header = T)
rownames(FF)<-FF[,1]
FF<-FF[,-1]
...
write.csv (cor_data_df, file =paste(gene,'correlation analysis.csv',sep=' '), row.names =FALSE)
...
dev.off()
pdf(file = paste(gene,'Positive Correlation Heatmap.pdf',sep=' '),width = 8,height = 6)
print(p)
dev.off()
...
p<-pheatmap(phemapdata,scale="row",cluster_row=F,cluster_col=F,legend= T,show_colnames = F,color = colorRampPalette(c("blue", "white", "red"))(50))
dev.off()
pdf(file = paste(gene,'Negative Correlation Heatmap.pdf',sep=' '),width = 8,height = 6)
print(p)
dev.off()
...
if(length(rownames(yy))!= 0) {
  ggsave(p1,filename = paste(gene,'GO_BPbubble plot.pdf',sep=' '),width = 7.23,height = 8)
  ggsave(p1,filename = paste(gene,'GO_BPbubble plot.TIFF',sep=' '),width = 7.23,height = 8)
}
...
if(length(rownames(yy))!= 0) {
  ggsave(p1,filename = paste(gene,'GO_CCbubble plot.pdf',sep=' '),width = 7.23,height = 8)
  ggsave(p1,filename = paste(gene,'GO_CCbubble plot.TIFF',sep=' '),width = 7.23,height = 8)
}
...
yy<-as.data.frame(y)
if(length(rownames(yy))!= 0){
  ggsave(p1,filename = paste(gene,'GO_MFbubble plot.pdf',sep=' '),width = 7.23,height = 8)
  ggsave(p1,filename = paste(gene,'GO_MFbubble plot.TIFF',sep=' '),width = 7.23,height = 8)
}

KEGG_result <- enrichKEGG(genelist_input, keyType = "kegg",pvalueCutoff=1,qvalueCutoff=1,pAdjustMethod = "BH", minGSSize = 5, maxGSSize = 500,organism = "hsa", use_internal_data=T)  
...
if(length(rownames(yy))!= 0){
  ggsave(p2,filename = paste(gene,'KEGGbubble plot.pdf',sep=' '),width = 7.23,height = 8)
  ggsave(p2,filename = paste(gene,'KEGGbubble plot.TIFF',sep=' '),width = 7.23,height = 8)
}
...
ggsave(p,filename = paste(gene,'GSEA_GOWave Plot.pdf',sep=' '),width = 8,height = 8)
ggsave(p,filename = paste(gene,'GSEA_GOWave Plot.TIFF',sep=' '),width = 8,height = 8)
p<-ridgeplot(KEGG_gseresult, 20) 
p<-p + scale_y_discrete(labels=function(x) str_wrap(x, width=50))
ggsave(p,filename = paste(gene,'GSEA_KEGGWave Plot.pdf',sep=' '),width = 8,height = 8)
ggsave(p,filename = paste(gene,'GSEA_KEGGWave Plot.TIFF',sep=' '),width = 8,height = 8)
p<-ridgeplot(Go_Reactomeresult, 20) 
p<-p + scale_y_discrete(labels=function(x) str_wrap(x, width=50))
ggsave(p,filename = paste(gene,'GSEA_ReactomeresultWave Plot.pdf',sep=' '),width = 8,height = 8)
ggsave(p,filename = paste(gene,'GSEA_ReactomeresultWave Plot.TIFF',sep=' '),width = 8,height = 8)
