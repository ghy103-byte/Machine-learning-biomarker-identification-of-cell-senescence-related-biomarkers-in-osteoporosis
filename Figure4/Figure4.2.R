...
library(tidyverse)
library(ggpubr)
rt<-read.csv("merge matrix new.csv",header = T,sep = ",",row.names = 1)
...
ggsave(p, filename =paste0("merge","DEGs plot.pdf"), width = 10, height = 6)

drawdata=drawdara
drawdata$condition<-factor(drawdata$condition,levels = c('OP','Control'))
my_comparisons <- list( c("OP", "Control") )
...
for (gene in Index) {
  ...
  ggsave(p,filename = paste(gene,'DEs.pdf',sep=' '),width = 5.6,height = 4.22)
  print(gene)
}
