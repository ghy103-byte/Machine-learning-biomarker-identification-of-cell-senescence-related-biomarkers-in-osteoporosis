...
inputtemp <- read.csv("merge matrix new.csv", header = T,sep = "," ,row.names = 1)
aaa<-read.table("venn gene from ML.txt",header = T)
aaa<-aaa[,1]
...
write.csv(cor_data,file = paste0(gene,"correlation result.csv"))
pdf(file=paste0(gene,"correlation with Immune cells.pdf"),width=8,height=7)
cor_data %>% 
  filter(p.value <0.05) %>% 
  ggplot(aes(cor,forcats::fct_reorder(cell,cor)))+
  geom_segment(aes(xend=0,yend=cell))+
